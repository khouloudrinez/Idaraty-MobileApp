<template>
  <ion-page>
    ...
    <ion-content v-if="list">
      ...
      <div class="procedure">
        <ion-img :src="list.imageSrc" :alt="list.title" class="procedure-image"></ion-img>
        <h2>{{ list.title }}</h2>
      </div>
      <ion-segment v-model="selectedSegment" v-show="list.resume || list.prerequis || list.etapes">
        <ion-segment-button value="resume" :checked="true" v-if="list.resume">
          Résumé
        </ion-segment-button>
        <ion-segment-button value="prerequis" v-if="list.prerequis">
          Pré-requis
        </ion-segment-button>
        <ion-segment-button value="etapes" v-if="list.etapes">
          Étapes
        </ion-segment-button>
      </ion-segment>
      <ion-list>
        <ion-item-group v-show="selectedSegment === 'resume' && list.resume && list.resume.length > 0">
          <ion-item>
            <p class="section-content">{{ list.resume }}</p>
          </ion-item>
        </ion-item-group>
        <ion-item-group v-show="selectedSegment === 'prerequis' && list.prerequis && list.prerequis.length > 0">
          <ion-item>
            <ion-list>
              <ion-item v-for="(prereq, index) in list.prerequis" :key="index" class="prereq-item">
                {{ prereq }}
              </ion-item>
            </ion-list>
          </ion-item>
        </ion-item-group>
        <ion-item-group v-show="selectedSegment === 'etapes' && list.etapes && list.etapes.length > 0">
          <ion-item>
            
            <ion-list>
              <ion-item v-for="(etape, index) in list.etapes" :key="index" class="etape-item">
                {{ etape }}
              </ion-item>
            </ion-list>
          </ion-item>
...
<script setup>
import { IonContent, IonPage, IonSegment, IonSegmentButton, IonList, IonItem, IonItemGroup, IonImg } from '@ionic/vue';
...
const selectedSegment = ref('resume'); 
const list = reactive({
  resume: null,
  prerequis: null,
  etapes: null,
...
watch(lists, (newLists) => {
  if (router.currentRoute.value.params && Array.isArray(newLists)) {
    id.value = router.currentRoute.value.params.id;
    
    const selectedItem = newLists.find(item => item.id === Number(id.value));
    Object.assign(list, selectedItem || {});
  }
});
...
onMounted(() => {
  if (router.currentRoute.value.params && Array.isArray(lists.value)) {
    id.value = router.currentRoute.value.params.id;
    const selectedItem = lists.value.find(item => item.id === Number(id.value));
    Object.assign(list, selectedItem || {});
  }
});
